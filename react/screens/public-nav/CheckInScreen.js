...
import {CheckInScreenHeader} from '../../components/CheckInScreenHeader';
...
class _CheckInScreen extends React.Component {
  static navigationOptions = ({navigation, screenProps, theme, navigationOptions}) => ({
    header: <CheckInScreenHeader navigation={navigation}/>,
  });
    ...
    return (
      <ContentContainer navigate={navigate} hasFooter={false}>
        <RoundedCornerPanel style={{flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
          <QRCode
            value={"https://mokirim.aldianfazrihady.com/" + this.props.currentLanguage + "/admin/checkin/checkin/add/?" + qs.stringify({
              code: booking.checkin_code,
              booking: booking.id,
              place: booking.schedule.from_place.id,
            })}
          />
          <Text>{booking.checkin_code}</Text>
        </RoundedCornerPanel>
      </ContentContainer>
    );
  }
...
export const CheckInScreen = connect(mapStateToProps, mapDispatchToProps)(_CheckInScreen);
